...
final class DashboardSignalListViewModel: ListViewModelProtocol {
    // MARK: - Variables
    var title = "Signals"
    
    var assetType: AssetType = .signal
    
    var signalListDelegateManager: DashboardSignalListDelegateManager!
    ...
    var viewModels = [CellViewAnyModel]() {
        didSet {
            ...
            self.activeViewModels = viewModels.filter { $0.signal.status != .archived }
            self.archiveViewModels = viewModels.filter { $0.signal.status == .archived }
        }
    }
    var activeViewModels = [SignalTableViewCellViewModel]()
    var archiveViewModels = [SignalTableViewCellViewModel]()
    ...
    init(withRouter router: DashboardRouter) {
        ...
        signalListDelegateManager = DashboardSignalListDelegateManager(with: self)
...
// MARK: - TableView
extension DashboardSignalListViewModel {
    // MARK: - Public methods
...
extension DashboardSignalListViewModel {
    ...
    func model(at assetId: String) -> CellViewAnyModel? {
        if activePrograms {
            if let i = activeViewModels.index(where: { $0.signal.id?.uuidString == assetId }) {
                return activeViewModels[i]
            }
        } else {
            if let i = archiveViewModels.index(where: { $0.signal.id?.uuidString == assetId }) {
    ...
    private func fetch(_ completionSuccess: @escaping (_ totalCount: Int, _ viewModels: [SignalTableViewCellViewModel]) -> Void, completionError: @escaping CompletionBlock) {
        ...
        let dashboardActionStatus: InvestorAPI.DashboardActionStatus_v10InvestorSignalsGet? = .all
